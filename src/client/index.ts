/**
 * juzi-at-mention client plugin: the browser half of the Codex-style @file
...
import { createAtSessionSource } from './sessionSource.ts'
...
export function apply(ctx: ClientContext): void {
  ...
  const reportSettingsError = (
    ...
    console.error(`[juzi-at-mention] settings ${operation} failed:`, error)
  }
  ...
  // Session mention source: a second '@' group listing this workspace's other
  // sessions by title (current and blank sessions excluded). Picking lands a
  // canonical `@[label](dsh-session:…)` token the Host resolves into a
  // read-only snapshot at the pre-step boundary.
  ctx.effect(() => {
  ...
  const search = async (sessionId: SessionId, signal: AbortSignal): Promise<readonly FileEntry[]> => {
    if (atFile === undefined) throw new Error('juzi-at-mention: the atFile Remote is not mounted')
  ...
  const openPath = (path: string): void => {
    void connection.api.host.openPath({ path }).then((response: OpenPathResponse) => {
      ...
      console.error('[juzi-at-mention] open failed:', error)
    })
  }
  ...
  ctx.slots.inject('conversation.input.overlay', () => ctx.slots.register({
    ...
    inject: (sessionId): FolderNavigatorInjected => {
      ...
      if (actx === undefined) throw new Error(`juzi-at-mention: session "${String(sessionId)}" has no client scope`)
