...
describe('juzi-at-mention host composition', () => {
  ...
  it('search indexes the addressed workspace, files and directories', async () => {
    const root = await mkdtemp(join(tmpdir(), 'juzi-at-mention-runtime-'))
  ...
  it('adds only the addressed workspace filters to the global list', async () => {
    const first = await mkdtemp(join(tmpdir(), 'juzi-at-mention-runtime-first-'))
    const second = await mkdtemp(join(tmpdir(), 'juzi-at-mention-runtime-second-'))
