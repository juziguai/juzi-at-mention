...
describe('juzi-at-mention invariant companion', () => {
  it('registers the package and withdraws the registration with its fiber', async () => {
    ...
    expect(() => registry.register('juzi-at-mention', () => {})).toThrow(/already registered/)
    await fiber.dispose()
    // After withdrawal the package name is free again.
    expect(() => registry.register('juzi-at-mention', () => {})).not.toThrow()
